...
export class EmailValidator {
  ...
  validate(email: string): Result<string, string> {
    ...
    if (typeof email !== 'string') {
      return Result.Err('Email must be a string');
    }
    ...
    if (!emailRegex.test(trimmedEmail)) {
      return Result.Err('Email format is invalid');
    }

    // Additional checks for common email issues
    if (trimmedEmail.length > 254) {
      return Result.Err('Email is too long (maximum 254 characters)');
    }
    ...
    if (trimmedEmail.startsWith('.') || trimmedEmail.endsWith('.')) {
      return Result.Err('Email cannot start or end with a dot');
    }

    return Result.Ok(trimmedEmail.toLowerCase());
  }
   ...
   */
  validateWithDisposableCheck(email: string): Result<string, string> {
